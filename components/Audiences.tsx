        ...
        {t.blocks.map((block, i) => {
          ...
          return (
            <StackingCardItem
              key={block.who}
              index={i}
              topPosition={`${11 + i * 2.2}%`}
              className="h-[max(80svh,520px)]"
            >
              <article className="relative flex h-[82%] flex-col overflow-hidden border rule-dark bg-raised px-6 py-7 shadow-[0_-24px_48px_-28px_rgba(0,0,0,0.55)] md:px-12 md:py-10">
                ...
                <div className="relative grid flex-1 content-center gap-7 py-8 lg:grid-cols-12 lg:gap-12">
                  <div className="lg:col-span-5">
                    <AudienceIcon
                      name={block.icon}
                      className="h-11 w-11 text-cerulean md:h-16 md:w-16"
                    />
                    <h3 className="display-tight mt-6 text-display-md text-frost">{block.who}</h3>
                  </div>
